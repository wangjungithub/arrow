...
SeleniumDriver.prototype.start = function (callback) {
    ...
    if (self.capabilities.error) {
        self.errorCallback(self.logger, self.capabilities.error, callback);
        return;
    }
    self.logger.trace("Capabilities :");
    self.logger.trace(self.capabilities);

    if ("phantomjs" === self.capabilities.browserName) {
        self.wdHubHost = self.config["phantomHost"];
        self.sessionId = "";
    } else {
        self.wdHubHost = self.config["seleniumHost"];
    }


    // android flag for minifying JS
    if ("android" === self.capabilities.browserName) {
        self.minifyJS = true;
    } else {
        self.minifyJS = false;
    }
    ...
    if (self.sessionId) {
        self.logger.info("Connecting to selenium: " + self.wdHubHost);
    } else {
        self.logger.info("Connecting to selenium: " + self.wdHubHost + ", browser: " + self.capabilities.browserName + ", version: " + self.capabilities.version);
    }
    ...
    self.wd = require(SeleniumDriver.wdAppPath);

    self.webdriver = this.createWebDriver(this.sessionId);

    // private listener for uncaught exception
    self.wd.promise.Application.getInstance().on('uncaughtException', function (e) {
...
SeleniumDriver.prototype.stop = function (callback, error) {
    // clean additional webdrivers
    for (var name in this.extra_webdrivers) {
        this.extra_webdrivers[name].quit();
        delete this.extra_webdrivers[name];
    }
 ...
 * @param testParams parameters for this test
 * @params callback function to call at the end of the test with errorMsg
 * @param web_driver webdriver to run test script 
 */
...
SeleniumDriver.prototype.createWebDriver= function (sessionid, capability) {
    var driver = new this.wd.Builder().
        usingServer(this.wdHubHost).
        usingSession(sessionid || this.sessionId).
        withCapabilities(capability || this.capabilities).build();

    driver.By = this.wd.By;
...
/**
 * Driver interface method
 * create additional webdriver object, to be used for page interaction
 *
 * @param name the name of additional webdriver
 * @param capability the capability of additional webdriver
 */
SeleniumDriver.prototype.createAdditionalWebDriver= function (name, capability) {
    var driver;

    if (!name) {
        this.logger.error("please specify additional webdriver name");
        return null;
    }

    if (this.extra_webdrivers.hasOwnProperty(name)) {
        driver = this.extra_webdrivers[name];
    } else {
        driver = this.createWebDriver(null, capability);
        this.extra_webdrivers[name] = driver;
    }
...
/**
 * Driver interface method
 * delete additional webdriver
 *
 * @param name the name of additional webdriver
 */
SeleniumDriver.prototype.deleteAdditionalWebDriver= function (name) {
    if (!name) {
        this.logger.error("please specify additional webdriver name");
        return;
    }
    ...
    if (this.extra_webdrivers.hasOwnProperty(name)) {
        ...
        this.logger.error("Cannot find webdriver: " + name);
    }
}
